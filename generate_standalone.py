...
def embedFileContent(content_svg, file_len, startind, input_path,
                     output_filename):
    """Embed the content of the script file.

    Replace the content of the first script tag found, starting from the
    input index.
    """
    # Messages strings
    ...
    while ((i != file_len) and ('/>' not in content_svg[i]) and
           ('</script>' not in content_svg[i])):
        i += 1
    if i == file_len:
    ...
    content_svg[closetag] = (
        content_svg[closetag].replace('/>', '>').replace(' >', '>'))
    ...
    if i > closetag:
        ...
        embedded_filename = content_svg[i].split('"')[1]
...
def createStandalone(input_filename, input_path, output_path):
    """Output a standalone SVG file based on the provided input SVG file."""
    # Output file
    ...
    ind = 0
    while ind != file_len:
        content_svg, ind = embedFileContent(content_svg, file_len, ind,
...
'''
   Do not edit the generated standalone files with Inkscape as the embedded
   scripts may be reformatted without the <![CDATA[ ... ]]> tag, which leads to
   increased files size and less readability of the embedded scripts.
'''
